...
for region, cfg in REGIONS.items():
    ...
    obs_df = table.to_dataframe(dense=True)  # ASVs x samples

    # Load FASTA → sequence to Feature ID
    seq_to_feature = {
        str(rec.seq): rec.id
        for rec in SeqIO.parse(cfg["fasta"], "fasta")
    }

    # Build ASV metadata table
    ...
    biom_out = cfg["biom"].replace(".biom", "_Gg2-genusASV.biom")
    with biom.util.biom_open(biom_out, "w") as f:
        renamed_table.to_hdf5(
            f,
            "Greengenes2 genus-ranked ASVs (abundance-based)"
        )

    print(f"  Features retained: {renamed_table.shape[0]}")
